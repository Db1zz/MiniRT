	...
	)
{
	...
	*u = (vec3_dot(diff, tan) / TEXTURE_SC) \
		- floor(vec3_dot(diff, tan) / TEXTURE_SC);
	*v = (vec3_dot(diff, bitan) / TEXTURE_SC) \
		- floor(vec3_dot(diff, bitan) / TEXTURE_SC);
}

t_color	get_color(t_texture *t, double u, double v, t_vector *bump)
{
	int	x;
	int	y;
	int	color;
